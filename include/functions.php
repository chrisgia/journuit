<?php
	...
	function deleteEintrag($db, $rtbUrl, $eintragId, $username, $userId){
		...
		if(isOwner($db, $userId, $rtbId)){
			$selectBilder = $db->prepare("SELECT bilder.id, bilder.file_ext FROM eintraege_bilder JOIN bilder ON (bilder.id = eintraege_bilder.bild_id) WHERE eintrag_id = ?");
	...
	// von allixsenos, http://ie2.php.net/strtr
